...
<?php ob_start(); ?>
<script src="../../../assets/js/specialty/list.js" defer></script>
<?php $js = ob_get_clean(); ?>

<?php $title = "KGB : missions | administration | specialties"?>

<?php ob_start(); ?>
<div class="container-fluid container-sm text-light">
    <div class="text-center my-5">
        <h1 class="text-uppercase font-monospace">Administration</h1>
        <h2 class="text-uppercase font-monospace">Specialty list</h2>
    </div>
    <div>
        <nav style="--bs-breadcrumb-divider: '>';" aria-label="breadcrumb">
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a class="link-secondary" href="?controller=administration">Home</a></li>
                <li class="breadcrumb-item active text-light" aria-current="page">Specialty list</li>
            </ol>
        </nav>
    </div>
    <!-- ADMIN INTERFACE -->
    <?php if (isset($_SESSION['admin']) && $_SESSION['admin'] == true) : ?>
        <div class="row justify-content-center gx-5 gy-3 mb-4">
            <div class="col-10 col-sm-7 col-md-6 col-lg-5">
                <form class="d-flex" action="" method="GET" role="search">
                    <input type="hidden" name="controller" value="specialty" />
                    <input type="hidden" name="action" value="list" />
                    <input class="form-control me-2" id="search-input" name="search" type="search"
                           placeholder="<?= $search == '' ? 'Search by name' : '' ?>"
                           value="<?= $search == '' ? '' : htmlspecialchars($search) ?>" aria-label="Search specialty by name">
            ...
            <div class="col-10 col-sm-5 col-md-6 col-lg-7">
                <a href="?controller=specialty&action=add" class="btn btn-secondary me-2">Add</a>
            ...
            <table class="table table-dark table-striped table-hover">
                ...
                <?php if (isset($specialties)): ?>
                    <?php foreach ($specialties as $specialty): ?>
                        <tr>
                            <td class="font-monospace"><input id="table-checkbox-<?= $specialty->getName(); ?>"
                                                              name="table-checkbox-<?= $specialty->getName(); ?>"
                                                              class="table-checkbox" type="checkbox"
                                                              value="<?= $specialty->getId(); ?>"></td>
                            <td class="font-monospace">
                                <?= $specialty->getId() ?>
                            </td>
                            <td class="font-monospace">
                                <a href="?controller=specialty&action=edit&id=<?= $specialty->getId(); ?>">
                                    <?= $specialty->getName(); ?>
                                </a>
                            </td>
                        </tr>
            ...
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <p>Do you really want to delete those specialties?</p>
                    </div>
        ...
        <div class="d-flex justify-content-center mb-5">
            <nav aria-label="Specialties page navigation">
                <ul class="pagination">
                    <li class="page-item <?= $pagination['currentPage'] == 1 ? 'disabled' : '' ?>">
                        <a class="page-link text-dark"
                           href="?controller=specialty&action=list&<?= $search != '' ? 'search=' . htmlspecialchars($search) . '&' : '' ?>page=<?= htmlspecialchars($pagination['currentPage'] - 1) ?>"
                           aria-label="Previous">
                    ...
                    <?php for ($page = 1; $page <= $pagination['nbPages']; $page++): ?>
                        <li class="page-item <?= $page == $pagination['currentPage'] ? 'active' : '' ?>">
                            <a class="page-link <?= $page == $pagination['currentPage'] ? 'bg-secondary border-secondary' : '' ?> text-dark"
                               href="?controller=specialty&action=list&<?= $search != '' ? 'search=' . htmlspecialchars($search) . '&' : '' ?>page=<?= htmlspecialchars($page) ?>"><?= htmlspecialchars($page) ?></a>
                        </li>
                    <?php endfor ?>
