...
<?php ob_start(); ?>
<script src="../../../assets/js/agent/list.js" defer></script>
<?php $js = ob_get_clean(); ?>

<?php $title = "KGB : missions | administration | agents"?>
    ...
    <?php if (isset($_SESSION['admin']) && $_SESSION['admin'] == true) : ?>
        <div class="row justify-content-center gx-5 gy-3 mb-4">
            <div class="col-10 col-sm-7 col-md-6 col-lg-5">
                <form class="d-flex" action="" method="POST" role="search">
                    <input class="form-control me-2" id="search-input" name="search" type="search"
                           placeholder="<?= $search == '' ? 'Search by code' : '' ?>"
                           value="<?= $search == '' ? '' : htmlspecialchars($search) ?>"
                           aria-label="Search agent by code">
            ...
            <div class="col-10 col-sm-5 col-md-6 col-lg-7">
                <a href="?controller=agent&action=add" class="btn btn-secondary me-2">Add</a>
                    ...
                    <?php foreach ($agents as $agent): ?>
                        <tr>
                            <td class="font-monospace"><input
                                        id="table-checkbox-<?= htmlspecialchars($agent->getCode()) ?>"
                                        name="table-checkbox-<?= htmlspecialchars($agent->getCode()) ?>"
                                        class="table-checkbox" type="checkbox"
                                        value="<?= htmlspecialchars($agent->getUUID()) ?>"></td>
                            <td class="font-monospace">
                                <a href="?controller=agent&action=edit&id=<?= htmlspecialchars($agent->getUUID()) ?>">
                                    <?= htmlspecialchars($agent->getUUID()) ?>
                                </a>
                            </td>
                            <?php
                            echo '<td class="font-monospace">' . htmlspecialchars($agent->getCode()) . '</td>';
                            echo '<td class="font-monospace">' . htmlspecialchars($agent->getFirstName()) . '</td>';
                            echo '<td class="font-monospace">' . htmlspecialchars($agent->getLastName()) . '</td>';
                            echo '<td class="font-monospace">' . htmlspecialchars($agent->getBirthday()) . '</td>';
                            echo '<td class="font-monospace">' . htmlspecialchars($agent->getNationality()) . '</td>';
                            ?>
                        </tr>
                    <?php endforeach ?>
                <?php endif ?>
                ...
                <ul class="pagination">
                    <li class="page-item <?= $pagination['currentPage'] == 1 ? 'disabled' : '' ?>">
                        <a class="page-link text-dark"
                           href="?controller=agent&action=list&<?= $search != '' ? 'search=' . htmlspecialchars($search) . '&' : '' ?>page=<?= htmlspecialchars($pagination['currentPage'] - 1) ?>"
                           aria-label="Previous">
                    ...
                    <?php for ($page = 1; $page <= $pagination['nbPages']; $page++): ?>
                        <li class="page-item <?= $page == $pagination['currentPage'] ? 'active' : '' ?>">
                            <a class="page-link <?= $page == $pagination['currentPage'] ? 'bg-secondary border-secondary' : '' ?> text-dark"
                               href="?controller=agent&action=list&<?= $search != '' ? 'search=' . htmlspecialchars($search) . '&' : '' ?>page=<?= htmlspecialchars($page) ?>"><?= htmlspecialchars($page) ?></a>
                        </li>
                    <?php endfor ?>
