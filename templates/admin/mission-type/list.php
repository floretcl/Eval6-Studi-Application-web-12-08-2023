...
<?php ob_start(); ?>
<script src="../../../assets/js/mission-type/list.js" defer></script>
<?php $js = ob_get_clean(); ?>

<?php $title = "KGB : missions | administration | missions type"?>
        ...
        <div class="row justify-content-center gx-5 gy-3 mb-4">
            <div class="col-10 col-sm-7 col-md-6 col-lg-5">
                <form class="d-flex" action="" method="GET" role="search">
                    <input type="hidden" name="controller" value="mission-type" />
                    <input type="hidden" name="action" value="list" />
                    <input class="form-control me-2" id="search-input" name="search" type="search"
                           placeholder="<?= $search == '' ? 'Search by name' : '' ?>"
                           value="<?= $search == '' ? '' : htmlspecialchars($search) ?>" aria-label="Search mission type by name">
            ...
            <div class="col-10 col-sm-5 col-md-6 col-lg-7">
                <a href="?controller=mission-type&action=add" class="btn btn-secondary me-2">Add</a>
                    ...
                    <?php foreach ($missionTypes as $missionType): ?>
                        <tr>
                            <td class="font-monospace"><input
                                        id="table-checkbox-<?= $missionType->getName(); ?>"
                                        name="table-checkbox-<?= $missionType->getName(); ?>"
                                        class="table-checkbox" type="checkbox"
                                        value="<?= $missionType->getId(); ?>"></td>
                            <td class="font-monospace">
                                <?= $missionType->getId() ?>
                            </td>
                            <td class="font-monospace">
                                <a href="?controller=mission-type&action=edit&id=<?= $missionType->getId(); ?>">
                                    <?= $missionType->getName(); ?>
                                </a>
                            </td>
                        </tr>
                ...
                <ul class="pagination">
                    <li class="page-item <?= $pagination['currentPage'] == 1 ? 'disabled' : '' ?>">
                        <a class="page-link text-dark"
                           href="?controller=mission-type&action=list&<?= $search != '' ? 'search=' . htmlspecialchars($search) . '&' : '' ?>page=<?= htmlspecialchars($pagination['currentPage'] - 1) ?>"
                           aria-label="Previous">
                    ...
                    <?php for ($page = 1; $page <= $pagination['nbPages']; $page++): ?>
                        <li class="page-item <?= $page == $pagination['currentPage'] ? 'active' : '' ?>">
                            <a class="page-link <?= $page == $pagination['currentPage'] ? 'bg-secondary border-secondary' : '' ?> text-dark"
                               href="?controller=mission-type&action=list&<?= $search != '' ? 'search=' . htmlspecialchars($search) . '&' : '' ?>page=<?= htmlspecialchars($page) ?>"><?= htmlspecialchars($page) ?></a>
                        </li>
                    <?php endfor ?>
