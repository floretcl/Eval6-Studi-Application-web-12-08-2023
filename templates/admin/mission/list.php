...
<?php ob_start(); ?>
<script src="../../../assets/js/mission/list.js" defer></script>
<?php $js = ob_get_clean(); ?>

<?php $title = "KGB : missions | administration | missions"?>
    ...
    <?php if (isset($_SESSION['admin']) && $_SESSION['admin'] == true) : ?>
        <div class="row justify-content-center gx-5 gy-3 mb-4">
            <div class="col-10 col-sm-7 col-md-6 col-lg-5">
                <form class="d-flex" action="" method="GET" role="search">
                    ...
                    <input class="form-control me-2" id="search-input" name="search" type="search"
                           ...
                           value="<?= $search == '' ? '' : htmlspecialchars($search) ?>" aria-label="Search mission by code name">
            ...
            <div class="col-10 col-sm-5 col-md-6 col-lg-7">
                <a href="?controller=mission&action=add" class="btn btn-secondary me-2">Add</a>
                    ...
                    <?php foreach ($missions as $mission): ?>
                        <tr>
                            <td class="font-monospace"><input
                                        id="table-checkbox-<?= $mission->getCodeName(); ?>"
                                        name="table-checkbox-<?= $mission->getCodeName(); ?>"
                                        class="table-checkbox" type="checkbox"
                                        value="<?= $mission->getUUID(); ?>"></td>
                            <td class="font-monospace">
                                <a href="?controller=mission&action=edit&id=<?= $mission->getUUID(); ?>">
                                    <?= $mission->getUUID(); ?>
                                </a>
                            </td>
                            <?php
                ...
                <ul class="pagination">
                    <li class="page-item <?= $pagination['currentPage'] == 1 ? 'disabled' : '' ?>">
                        <a class="page-link text-dark"
                           href="?controller=mission&action=list&<?= $search != '' ? 'search=' . htmlspecialchars($search) . '&' : '' ?>page=<?= htmlspecialchars($pagination['currentPage'] - 1) ?>"
                           aria-label="Previous">
                    ...
                    <?php for ($page = 1; $page <= $pagination['nbPages']; $page++): ?>
                        <li class="page-item <?= $page == $pagination['currentPage'] ? 'active' : '' ?>">
                            <a class="page-link <?= $page == $pagination['currentPage'] ? 'bg-secondary border-secondary' : '' ?> text-dark"
                               href="?controller=mission&action=list&<?= $search != '' ? 'search=' . htmlspecialchars($search) . '&' : '' ?>page=<?= htmlspecialchars($page) ?>"><?= htmlspecialchars($page) ?></a>
                        </li>
                    <?php endfor ?>
