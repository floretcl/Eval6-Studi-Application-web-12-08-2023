...
<?php ob_start(); ?>
<script src="../../../assets/js/contact/list.js" defer></script>
<?php $js = ob_get_clean(); ?>

<?php $title = "KGB : missions | administration | contacts"?>

<?php ob_start(); ?>
<div class="container-fluid container-sm text-light">
    <div class="text-center my-5">
        <h1 class="text-uppercase font-monospace">Administration</h1>
        <h2 class="text-uppercase font-monospace">Contact list</h2>
    </div>
    <div>
        <nav style="--bs-breadcrumb-divider: '>';" aria-label="breadcrumb">
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a class="link-secondary" href="?controller=administration">Home</a></li>
                <li class="breadcrumb-item active text-light" aria-current="page">Contact list</li>
            </ol>
        </nav>
    </div>
    <!-- ADMIN INTERFACE -->
    <?php if (isset($_SESSION['admin']) && $_SESSION['admin'] == true) : ?>
        <div class="row justify-content-center gx-5 gy-3 mb-4">
            ...
            <div class="col-10 col-sm-5 col-md-6 col-lg-7">
                <a href="?controller=contact&action=add" class="btn btn-secondary me-2">Add</a>
                    ...
                    <?php foreach ($contacts as $contact): ?>
                        <tr>
                            <td class="font-monospace"><input
                                        id="table-checkbox-<?= $contact->getCodeName(); ?>"
                                        name="table-checkbox-<?= $contact->getCodeName(); ?>"
                                        class="table-checkbox" type="checkbox"
                                        value="<?= $contact->getUUID(); ?>"></td>
                            <td class="font-monospace">
                                <a href="?controller=contact&action=edit&id=<?= $contact->getUUID(); ?>">
                                    <?= $contact->getUUID(); ?>
                                </a>
                            </td>
                            <?php
            ...
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <p>Do you really want to delete those contacts?</p>
                    </div>
                ...
                <ul class="pagination">
                    <li class="page-item <?= $pagination['currentPage'] == 1 ? 'disabled' : '' ?>">
                        <a class="page-link text-dark"
                           href="?controller=contact&action=list&<?= $search != '' ? 'search=' . htmlspecialchars($search) . '&' : '' ?>page=<?= htmlspecialchars($pagination['currentPage'] - 1) ?>"
                           aria-label="Previous">
                    ...
                    <?php for ($page = 1; $page <= $pagination['nbPages']; $page++): ?>
                        <li class="page-item <?= $page == $pagination['currentPage'] ? 'active' : '' ?>">
                            <a class="page-link <?= $page == $pagination['currentPage'] ? 'bg-secondary border-secondary' : '' ?> text-dark"
                               href="?controller=contact&action=list&<?= $search != '' ? 'search=' . htmlspecialchars($search) . '&' : '' ?>page=<?= htmlspecialchars($page) ?>"><?= htmlspecialchars($page) ?></a>
                        </li>
                    <?php endfor ?>
