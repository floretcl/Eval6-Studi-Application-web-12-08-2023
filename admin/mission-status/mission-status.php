...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $missionStatusId = $_POST['mission-status-id'];
    $missionStatusName = $_POST['mission-status-name'];

    try {
        // Mission status update request
        $sql = 'UPDATE Mission_status
            SET 
            Mission_status.mission_status_id = :id,
            Mission_status.mission_status_name = :name
            WHERE mission_status_id = :id';
        $statement = $pdo->prepare($sql);
        $statement->bindParam(':id', $missionStatusId, PDO::PARAM_STR);
        $statement->bindParam(':name', $missionStatusName, PDO::PARAM_STR);
        ...
        $message = "Error: unable to update mission status";
    }
}
...
try {
    ...
    if ($statement->execute()) {
        $missionStatus = $statement->fetchObject('MissionStatus');
    } else {
        $error = $statement->errorInfo();
        $logFile = '../../logs/errors.log';
        error_log('Error : ' . $error);
    ...
    $message = "Error: unable to display mission status details";
}
?>
...
<head>
    ...
    <title>KGB : missions | administration | mission status</title>
</head>
    ...
    <main>
        <div class="container-fluid container-sm text-light">
            <div class="text-center my-5">
                ...
                <h2 class="text-uppercase font-monospace">Edit mission status</h2>
            </div>
            <div>
                <nav style="--bs-breadcrumb-divider: '>';" aria-label="breadcrumb">
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item"><a class="link-secondary" href="../../admin.php">Home</a></li>
                        <li class="breadcrumb-item"><a class="link-secondary" href="./list.php">Mission status list</a></li>
                ...
                <div class="row mt-2 pb-2 mb-4 overflow-x-scroll">
                    <form action="" method="post">
                        <?php if (isset($message)) : ?>
                            <div class="alert <?= $message == 'Mission status updated' ? 'alert-success' : 'alert-danger' ?> d-flex align-items-center"
                                 role="alert">
                                <?php if ($message == 'Mission status updated') : ?>
                        ...
                        <?php endif ?>
                        <div class="mb-3">
                            <label for="mission-status-id" class="form-label">Id :</label>
                            <input type="text" class="form-control" id="mission-status-id" name="mission-status-id"
                                   value="<?= $missionStatus->getId() ?>" aria-describedby="mission-status-id-help"
                                   readonly required>
                            <div id="mission-status-id-help" class="form-text text-light">Read only.</div>
                        </div>
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" id="mission-status-name" name="mission-status-name"
                                   value="<?= $missionStatus->getName() ?>" maxlength="50" aria-describedby="mission-status-name-help"
                                   required>
                            <div id="mission-status-name-help" class="form-text text-light">Required. 50 characters max.</div>
                        </div>
                    ...
                    <div class="modal-dialog modal-dialog-centered">
                        <div class="modal-content">
                            ...
                            <div class="modal-body">
                                <p>Do you really want to delete this mission status?</p>
                            </div>
...
<script src="../../assets/js/mission-status/mission-status.js"></script>
</body>
